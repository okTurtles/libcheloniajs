...
import { DEFAULT_SNAPSHOT_INTERVAL, defaultJournalConfig } from './journal.js'
  ...
  'chelonia/configure': async function (this: CheloniaContext, config: CheloniaConfig) {
    ...
    if (journalOverride === null) {
      ...
      if (typeof journalOverride !== 'object' || Array.isArray(journalOverride)) {
        throw new TypeError(
          '[chelonia][journal] config.journal must be an object, `null` to ' +
      ...
      if (journalOverride.redactions !== undefined) {
        ...
        staged.redactions = journalOverride.redactions.map((r, i) => {
          ...
          if (
            ...
            throw new TypeError(
              `[chelonia][journal] config.journal.redactions[${i}] must be ` +
