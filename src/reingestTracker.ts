...
const trackers: Map<string, Set<TrackedEntry>> = new Map()

const ensure = (contractID: string): Set<TrackedEntry> => {
...
export const noteFutureEvent = (
  ...
  for (const entry of s) {
    if (entry.hash === hash) return 'duplicate'
  }
  if (s.size >= REINGEST_PER_CONTRACT_CAP) {
    throw new ChelErrorUnrecoverable(
      `more than ${REINGEST_PER_CONTRACT_CAP} different bad previousHEAD errors for contract ${contractID}`
    )
  }
  s.add({ hash, height })
  return 'added'
 ...
 */
export const noteReingestSuccess = (contractID: string, hash: string): boolean => {
  const s = trackers.get(contractID)
  if (!s) return false
  for (const entry of s) {
    if (entry.hash === hash) {
      s.delete(entry)
      if (s.size === 0) trackers.delete(contractID)
      return true
    }
  }
  return false
 ...
 */
export const pruneStaleEntries = (contractID: string, processedHeight: number): number => {
  const s = trackers.get(contractID)
  if (!s) return 0
  let pruned = 0
  for (const entry of s) {
...
export const hasPendingReingest = (contractID: string, hash: string): boolean => {
  const s = trackers.get(contractID)
  if (!s) return false
  for (const entry of s) {
...
export const pendingReingestCount = (contractID: string): number => {
 ...
 */
export const clearReingestTrackerForContract = (contractID: string): void => {
 ...
 */
export const clearReingestTrackerAll = (): void => {
  trackers.clear()
}
