...
| `redactions` | `{ path, redact }[]` | `[]` | Applied to both the before- and after-state **before diffing**. `path` uses dotted segments; `*` matches any key/index. `redact(value, fullPath, contractName)` MUST be pure and return a JSON-safe replacement (non-JSON-safe results are substituted with `REDACTION_UNSERIALIZABLE_SENTINEL`). Deep-copied on the way in. Overlapping paths are supported; redactors run in array order, so the projected output remains order-sensitive. |
