...
| `ChelErrorUnexpectedHttpResponseCode` | Server returned an unexpected HTTP status. The status is on `.cause` as a number, so callers can branch on it without parsing the message. Thrown by `handleFetchResult`, by a failed publish, by `chelonia/kv/get` and `chelonia/kv/set`, by `chelonia/out/ownResources`, and by `chelonia/out/nameToContractID`. `ChelErrorResourceGone` is a subclass, so it carries the status the same way. |
