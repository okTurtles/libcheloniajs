...
| `ChelErrorUnexpectedHttpResponseCode` | Server returned an unexpected HTTP status. The status is on `.cause` as a number, so callers can branch on it without parsing the message. Thrown by `handleFetchResult` and by a failed publish. |
