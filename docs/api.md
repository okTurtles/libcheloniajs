...
| `JournalEntry` | `src/types.ts` | Discriminated union of `{ kind: 'snapshot', ..., state }` and `{ kind: 'patch', ..., patch }`. Both arms may carry `error?` (the event itself failed to process) plus `diffError?` / `redactionError?` (the journal could not record what changed). See [`journal.md`](./journal.md#public-selectors). |
