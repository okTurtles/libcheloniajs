...
Chelonia can keep an optional, opt-in per-contract journal of every event
applied to a contract's state: an RFC-6902 JSON Patch diff per event plus
periodic full snapshots, so HEAD state stays reconstructible. Enable and
tune it through the `journal` block of `chelonia/configure`.

Full documentation:

- Guide (why journal, redactions, snapshot tuning, reconstruct, storage and
  leakage caveats): [docs/journal.md](docs/journal.md)
- Config fields and reconfigure rules (per-field `null` handling,
  `journal: null` reset): [docs/configure.md](docs/configure.md#journal-configuration)
- Public selectors: `chelonia/journal/get`, `chelonia/journal/reconstruct`,
  `chelonia/journal/clear`

### KV slots

Declarative typed key/value API layered on the server-side KV store.
Consumers register slots via `chelonia/kv/defineSlot` (or inline under the
`kv` key of `chelonia/defineContract`); the library manages a local mirror
(`rootState._kv`), pubsub filter coalescing, conflict retries, and schema
