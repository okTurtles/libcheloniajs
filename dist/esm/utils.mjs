...
import { ChelErrorForkedChain, ChelErrorResourceGone, ChelErrorUnexpectedHttpResponseCode, ChelErrorWarning } from './errors.mjs';
import { CONTRACT_IS_PENDING_KEY_REQUESTS } from './events.mjs';
...
export const findKeyIdByName = (state, name) => state._vm?.authorizedKeys &&
    Object.values(state._vm.authorizedKeys).find((k) => k.name === name && k._notAfterHeight == null)
        ?.id;
export const findForeignKeysByContractID = (state, contractID) => state._vm?.authorizedKeys &&
    Object.values(state._vm.authorizedKeys)
        .filter((k) => k._notAfterHeight == null && k.foreignKey?.includes(contractID))
        .map((k) => k.id);
export const findRevokedKeyIdsByName = (state, name) => state._vm?.authorizedKeys &&
    Object.values(state._vm.authorizedKeys || {})
...
export const validateKeyPermissions = (msg, config, state, signingKeyId, opT, opV) => {
    ...
    if (opT === SPMessage.OP_ACTION_UNENCRYPTED &&
...
export const recreateEvent = (entry, state, contractsState, disableAutoDedup) => {
        ...
        const recreateOperationInternal = (opT, opV) => {
            ...
            if (opT === SPMessage.OP_KEY_ADD) {
                ...
                newOpV = opV.filter((k) => {
                    ...
                    return (!has(state._vm.authorizedKeys, kId) ||
...
export function verifyShelterAuthorizationHeader(authorization, rootState) {
    ...
    if (!rootState)
        rootState = sbp('chelonia/rootState');
 ...
 */
export const httpErrorDetail = async (r) => {
    try {
