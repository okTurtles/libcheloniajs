...
export const keyAdditionProcessor = function (_msg, hash, keys, state, contractID, _signingKey, internalSideEffectStack) {
    ...
    for (const wkey of keys) {
        ...
        if (key.name.startsWith('#inviteKey-')) {
            ...
            const inviteSecret = decryptedKey || (has(this.transientSecretKeys, key.id)
...
export const recreateEvent = (entry, state, contractsState, disableAutoDedup) => {
    ...
    entry = SPMessage.cloneWith(head, newOp, { previousKeyOp, previousHEAD, height: previousHeight + 1 });
    return entry;
};
