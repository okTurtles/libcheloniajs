...
import { CURVE25519XSALSA20POLY1305, EDWARDS25519SHA512BATCH, deserializeKey, keyId, keygen, keygenOfSameType, serializeKey } from '@chelonia/crypto';
import sbp from '@sbp/sbp';
import { SPMessage } from './SPMessage.mjs';
import { encryptedDataKeyId, encryptedOutgoingData, encryptedOutgoingDataWithRawKey, isEncryptedData } from './encryptedData.mjs';
import { ChelErrorKeyNameNotFound, ChelErrorKeySpecInvalid, ChelErrorKeyWrapCycle } from './errors.mjs';
import { Secret } from './Secret.mjs';
import { copiedExistingData, findKeyIdByName, findSuitableSecretKeyId } from './utils.mjs';
// ---------------------------------------------------------------------------
...
export const keyUpdateSpec = (alias, spec = {}) => {
    if (!alias || typeof alias !== 'string') {
...
export const isKeyUpdateSpec = (value) => !!value && !!Object.getPrototypeOf(value)
    ?._isKeyUpdateSpec;
// ---------------------------------------------------------------------------
...
const typeSupportsPurpose = (type, purpose) => {
    if (purpose === 'enc')
        return type === CURVE25519XSALSA20POLY1305;
...
const inferTypeFromPurpose = (purpose) => {
    ...
    if (hasEnc && hasSigOrSak) {
        throw new ChelErrorKeySpecInvalid(`Cannot infer a key type for purpose ${JSON.stringify(purpose)}: ` +
    ...
    throw new ChelErrorKeySpecInvalid(`Cannot infer a key type for empty purpose ${JSON.stringify(purpose)}`);
};
const defaultPurposeForType = (type) => type === CURVE25519XSALSA20POLY1305 ? ['enc'] : ['sig'];
...
// entries.
export const normalizeKeySpecs = (input) => {
    const entries = [];
    const seen = new Set();
    if (Array.isArray(input)) {
        for (const item of input) {
            if (!isKeySpec(item)) {
                throw new ChelErrorKeySpecInvalid('Array key specs must be created with keySpec(alias, spec); ' +
...
export const normalizeKeyUpdateSpecs = (input) => {
    const entries = [];
    const seen = new Set();
    if (Array.isArray(input)) {
        for (const item of input) {
            if (!isKeyUpdateSpec(item)) {
                throw new ChelErrorKeySpecInvalid('Array key update specs must be created with keyUpdateSpec(alias, spec)');
            }
            if (seen.has(item.alias)) {
...
export const resolveGeneratedKeyReference = (keyMap, id, name, label, required = true) => {
    ...
    if (id != null) {
        if (name != null) {
            ...
            if (resolved !== id) {
                throw new ChelErrorKeyNameNotFound(`${label}: key name '${name}' resolves to ${resolved} but key ID ${id} was given`);
            }
        }
        return id;
    }
...
export const resolveStateKeyReference = (state, id, name, label, required = true) => {
    if (id == null && name == null) {
        if (required) {
            throw new TypeError(`${label}: either a key ID or a key name must be provided`);
        }
    ...
    if (id != null) {
        if (name != null) {
            ...
            if (currentId !== id) {
                throw new ChelErrorKeyNameNotFound(`${label}: key name '${name}' resolves to ${String(currentId)} ` +
    ...
    if (!state) {
        throw new ChelErrorKeyNameNotFound(`${label}: cannot resolve key name '${name}' because the contract state ` +
            'is not loaded');
    }
    const currentId = findKeyIdByName(state, name);
    if (!currentId) {
        throw new ChelErrorKeyNameNotFound(`${label}: no active key with name '${name}' in contract state`);
    }
    return currentId;
};
...
export const expandKeySpecs = (params) => {
    ...
    const register = (m) => {
        ...
        if (byId.has(m.id)) {
            throw new ChelErrorKeySpecInvalid(`Duplicate key ID ${m.id} (generated for both '${byId.get(m.id).alias}' ` +
                `and '${m.alias}')`);
        }
    ...
    for (const { alias, spec } of entries) {
        ...
        if (isReservedName(wireName) && !isKnownReservedName(wireName)) {
            throw new ChelErrorKeySpecInvalid(`Unknown reserved key name '${wireName}': the '#' namespace is ` +
        ...
        if (spec.key != null && spec.type != null) {
            throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': 'key' and 'type' are mutually exclusive`);
        }
        if (spec.key != null && spec.data != null) {
            throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': 'key' and 'data' are mutually exclusive ` +
        ...
        if (spec.foreignKeyFrom != null) {
            ...
            if (!originID || !originKeyName) {
                throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': foreignKeyFrom must be [contractID, keyName]`);
            }
            if (spec.key != null || spec.type != null || spec.data != null) {
                throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': foreignKeyFrom cannot be combined with 'key', ` +
                    "'type' or 'data' (the public half is copied from the origin contract)");
            }
            if (spec.encryptWith != null) {
                throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': foreignKeyFrom produces no secret material, so ` +
                    "'encryptWith' cannot be used");
            }
            if (!getContractState) {
                throw new ChelErrorKeyNameNotFound(`Key spec '${alias}': foreignKeyFrom requires contract state access ` +
            ...
            if (!originState?._vm?.authorizedKeys) {
                throw new ChelErrorKeyNameNotFound(`Key spec '${alias}': origin contract ${originID} is not loaded; ` +
                    'retain and sync it before building foreign keys from it');
            }
            const originKeyId = findKeyIdByName(originState, originKeyName);
            if (!originKeyId) {
                throw new ChelErrorKeyNameNotFound(`Key spec '${alias}': no active key named '${originKeyName}' in ` +
        ...
        if (spec.foreignKey != null) {
            if (spec.data == null) {
                throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': a direct 'foreignKey' declaration requires 'data' ` +
                    '(the public half of the origin key)');
            }
            if (spec.encryptWith != null) {
                throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': a foreign key declaration has no secret material, ` +
        ...
        if (isInvite && spec.quantity == null && spec.meta?.quantity == null) {
            throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': '#inviteKey*' specs require 'quantity'`);
        }
        ...
        if (spec.key != null) {
            ...
            throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': nothing to derive the key from — provide one of ` +
                "'key', 'type', 'purpose' or 'data'");
        }
        if (type !== EDWARDS25519SHA512BATCH &&
            type !== CURVE25519XSALSA20POLY1305) {
            throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': unsupported contract key type '${type}' ` +
        ...
        for (const p of purpose) {
            if (!typeSupportsPurpose(type, p)) {
                throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': purpose '${p}' is not compatible with key type '${type}'`);
            }
        }
    ...
    const assertEncPurpose = (key, label) => {
        if (!key || !Array.isArray(key.purpose) || !key.purpose.includes('enc')) {
            throw new ChelErrorKeySpecInvalid(`encryptWith references '${label}', which is not an encryption key`);
        }
    };
    const resolveWrapTarget = (alias, wrapWith) => {
        if (typeof wrapWith === 'string') {
            ...
            throw new ChelErrorKeyNameNotFound(`Key spec '${alias}': encryptWith references '${wrapWith}', which is ` +
                'neither another key in the same set nor an active key in the target contract');
        }
        if ('key' in wrapWith) {
            if (!wrapWith.key) {
                throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': encryptWith was given an empty raw key`);
            }
        ...
        if (!state?._vm?.authorizedKeys) {
            throw new ChelErrorKeyNameNotFound(`Key spec '${alias}': encryptWith references contract ` +
                `${wrapWith.contractID}, which is not loaded`);
        }
        const wrapperId = findKeyIdByName(state, wrapWith.name);
        if (!wrapperId) {
            throw new ChelErrorKeyNameNotFound(`Key spec '${alias}': no active key named '${wrapWith.name}' in contract ` +
                wrapWith.contractID);
        }
    ...
    for (const m of materialized) {
        ...
        if (target) {
            if (!target.hasSecret) {
                throw new ChelErrorKeySpecInvalid(`Key spec '${m.alias}': encryptWith references '${m.wrapWith}', which ` +
    ...
    for (const m of materialized) {
        if (m.wrapWith == null)
            continue;
        if (!m.hasSecret) {
            throw new ChelErrorKeySpecInvalid(`Key spec '${m.alias}': 'encryptWith' requires secret material, but this ` +
...
const deserializeTypeOf = (data, alias) => {
    try {
        ...
        throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': 'data' is not a valid serialized public key`, { cause: e });
    }
};
const requireRingLevel = (spec, alias, isSak, isInvite) => {
    ...
    throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': 'ringLevel' is required for ordinary key names — ` +
...
export const expandKeyUpdateSpecs = (params) => {
    ...
    for (const { alias, spec } of entries) {
        ...
        if (spec.oldKeyId == null && oldKeyName == null) {
            throw new ChelErrorKeySpecInvalid(`Key update spec '${alias}': either 'oldKeyId' or 'oldKeyName' must be provided`);
        }
        let oldKeyId;
        if (spec.oldKeyId != null) {
            ...
            if (oldKeyName != null) {
                ...
                if (byName !== oldKeyId) {
                    throw new ChelErrorKeyNameNotFound(`Key update spec '${alias}': oldKeyName '${oldKeyName}' resolves ` +
            ...
            if (!oldKeyId) {
                throw new ChelErrorKeyNameNotFound(`Key update spec '${alias}': no active key named '${oldKeyName}' ` +
                    `in contract ${contractID}`);
            }
        }
        const existing = authorizedKeys[oldKeyId];
        if (!existing) {
            throw new ChelErrorKeyNameNotFound(`Key update spec '${alias}': old key ${oldKeyId} does not exist in ` +
                `contract ${contractID}`);
        }
        if (existing._notAfterHeight != null) {
            throw new ChelErrorKeyNameNotFound(`Key update spec '${alias}': old key ${oldKeyId} has been revoked`);
        }
        if (spec.name != null && spec.name !== existing.name) {
            throw new ChelErrorKeyNameNotFound(`Key update spec '${alias}': 'name' is an optional consistency assertion ` +
        ...
        if (spec.rotate === true) {
            if (spec.key != null) {
                throw new ChelErrorKeySpecInvalid(`Key update spec '${alias}': 'rotate' and 'key' are mutually exclusive`);
            }
            ...
            if (newKey.type !== deserializeKey(existing.data).type) {
                throw new ChelErrorKeySpecInvalid(`Key update spec '${alias}': replacement key type '${newKey.type}' ` +
    ...
    const updates = resolved.map((r) => {
        ...
        if (spec.encryptWith != null && explicitRawWrapper == null) {
            throw new ChelErrorKeySpecInvalid(`Key update spec '${r.alias}': 'encryptWith' in update specs only ` +
                'supports { key } targets');
        }
        if (spec.encryptWith != null && secret == null) {
            throw new ChelErrorKeySpecInvalid(`Key update spec '${r.alias}': 'encryptWith' requires a replacement key ` +
                "('rotate' or 'key')");
        }
        ...
        if (secret != null) {
            ...
            if (!wrapped) {
                ...
                // have no wrapper.
                if (!transient) {
                    throw new ChelErrorKeySpecInvalid(`Key update spec '${r.alias}': the existing key has no wrapped secret. A ` +
...
export const storeGeneratedSecretKeys = (ctx, keys) => {
    ...
    'chelonia/key/rotate': async function (params) {
        ...
        if (names === '*') {
            ...
            selected = names.map((name) => {
                ...
                if (!keyId) {
                    throw new ChelErrorKeyNameNotFound(`chelonia/key/rotate: no active key named '${name}' in ${contractID}`);
                }
        ...
        // can be rotated.
        selected = selected.filter((k) => k.meta?.private?.content != null && sbp('chelonia/haveSecretKey', k.id));
        ...
        let signingKeyId = hasExplicitSigner
            ? resolveStateKeyReference(state, params.signingKeyId ?? null, params.signingKeyName ?? null, 'chelonia/key/rotate signingKey')
            : undefined;
        if (signingKeyId == null) {
            ...
            signingKeyId = findSuitableSecretKeyId(state, usesAdditionalOps
                ? [SPMessage.OP_ATOMIC, SPMessage.OP_KEY_UPDATE]
                : [SPMessage.OP_KEY_UPDATE], ['sig'], minRingLevel) ?? undefined;
        }
        if (!signingKeyId) {
