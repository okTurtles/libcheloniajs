...
const normalizeSpecs = (input, isMarked, kind) => {
    ...
    const add = (alias, spec) => {
        ...
        if (spec == null || typeof spec !== 'object') {
            throw new ChelErrorKeySpecInvalid(`${kind.paramName}: '${alias}' must be a ${kind.noun} object`);
        }
    ...
    if (Array.isArray(input)) {
        for (const item of input) {
            if (!isMarked(item)) {
                throw new ChelErrorKeySpecInvalid(`Array ${kind.noun}s must be created with ${kind.factory}(alias, spec)` +
...
export const normalizeKeySpecs = (input) => normalizeSpecs(input, isKeySpec, {
    noun: 'key spec',
    paramName: 'keys',
    mapType: 'KeySpecMap',
    factory: 'keySpec',
...
export const normalizeKeyUpdateSpecs = (input) => normalizeSpecs(input, isKeyUpdateSpec, {
...
export const expandKeySpecs = (params) => {
    ...
    for (const { alias, spec } of entries) {
        ...
        if (spec.data != null && spec.type != null) {
            throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': 'data' and 'type' are mutually exclusive ` +
        ...
        if (spec.meta?.private?.content != null) {
            throw new ChelErrorKeySpecInvalid(`Key spec '${alias}': 'meta.private.content' cannot be set on a key ` +
