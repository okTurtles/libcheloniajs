...
export default sbp('sbp/selectors/register', {
    ...
    'chelonia/haveSecretKey': function (keyId, persistent) {
        if (!persistent && has(this.transientSecretKeys, keyId))
            return true;
        const rootState = sbp(this.config.stateSelector);
        ...
        this.pubsub = createClient(pubsubURL, {
            ...
            messageHandlers: {
                ...
                [NOTIFICATION_TYPE.ENTRY](msg) {
                    ...
                    // Calling via SBP also makes it simple to implement 'test/backend.mjs'
    ...
    'chelonia/contract/withRetained': async function (contractIDs, callback) {
        ...
        try {
            ...
            await sbp('chelonia/contract/release', contractIDs, { ephemeral: true }).catch((e) => {
    ...
    'chelonia/in/deserializeMessage': function (rawMessage, options = {}) {
        if (options.headOnly) {
            ...
            return SPMessage.deserialize(rawMessage, this.transientSecretKeys, options.state, this.config.unwrapMaybeEncryptedData);
        }
    },
    'chelonia/in/processMessage': function (messageOrRawMessage, state) {
        const stateCopy = cloneDeep(state);
        const message = typeof messageOrRawMessage === 'string'
            ? SPMessage.deserialize(messageOrRawMessage, this.transientSecretKeys, stateCopy, this.config.unwrapMaybeEncryptedData)
            : messageOrRawMessage;
    ...
    'chelonia/latestContractState': async function (contractID, options = { forceSync: false }) {
        ...
        for (;;) {
            ...
            try {
                ...
                if (e instanceof ChelErrorUnrecoverable)
                    throw e;
                state = stateCopy;
            }
        }
    ...
    'chelonia/out/keyDel': async function (params) {
        ...
        const payload = data
            ...
            if (isEncryptedData(keyId))
                return keyId;
            if (!has(state._vm.authorizedKeys, keyId) ||
