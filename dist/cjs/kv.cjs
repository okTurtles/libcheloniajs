...
function setSlotStatus(ctx, rootState, contractID, contractType, key, status, lastError) {
    ...
    if (statusUnchanged && !lastError && !entry.lastError)
        return;
...
exports.default = (0, sbp_1.default)('sbp/selectors/register', {
    ...
    'chelonia/kv/defineSlot': function (def) {
        ...
        const resolvedDefault = typeof dv === 'function' ? dv() : dv;
        for (const contractType of types) {
            ...
            for (const cID of this.subscriptionSet) {
                ...
                if (!meta || meta.type !== contractType)
                    continue;
    ...
    'chelonia/kv/_reconcileForSlot': function (slot, contractID) {
        ...
        if (!meta || meta.type !== slot.contractType)
            return;
    ...
    'chelonia/kv/_loadSlot': function ({ contractID, slot, reason }) {
        return (0, sbp_1.default)('chelonia/queueInvocation', contractID, async () => {
            ...
            setSlotStatus(this, rootState, contractID, slot.contractType, slot.key, 'loaded');
    ...
    // later task step).
    'chelonia/kv/_onContractsModified': function ({ added }) {
    ...
    'chelonia/kv/_handleRemote': function (contractID, key, parsed) {
        ...
        if (unwrapped === null) {
            ...
            try {
                ...
                setSlotStatus(this, rootState, contractID, slot.contractType, key, 'error', lastError);
                return Promise.resolve();
            }
        }
        else {
    ...
    //     failures — §4.9).
    'chelonia/kv/update': async function ({ contractID, key, updater, value, maxAttempts, signal, ifMatch }) {
        ...
        if (!contractMeta || !this.subscriptionSet.has(contractID)) {
            throw new errors_js_1.ChelErrorKvSlotUnknown(`[chelonia/kv] update: contract ${contractID} is not synced`);
        }
        const contractType = contractMeta.type;
        if (typeof contractType !== 'string') {
            throw new errors_js_1.ChelErrorKvSlotUnknown(`[chelonia/kv] update: contract ${contractID} has no resolved type`);
        }
        const slot = this.kvSlots.get(registryKey(contractType, key));
        if (!slot) {
            throw new errors_js_1.ChelErrorKvSlotUnknown(`[chelonia/kv] update: no slot registered for ${contractType}::${key}`);
        }
        ...
        if (hasUpdater) {
            ...
            reducer = slot.defaultUpdater(value);
        }
        ...
        const onconflict = async ({ currentData, etag }) => {
            ...
            return [{ __chelKvNonce: nonce, value: validated }, etag ?? ''];
        };
    ...
    'chelonia/kv/read': function (contractID, key) {
        ...
        const contractMeta = rootState.contracts?.[contractID];
        if (!contractMeta || !this.subscriptionSet.has(contractID)) {
            throw new errors_js_1.ChelErrorKvSlotUnknown(`[chelonia/kv] read: contract ${contractID} is not synced`);
        }
        const contractType = contractMeta.type;
        if (typeof contractType !== 'string') {
            throw new errors_js_1.ChelErrorKvSlotUnknown(`[chelonia/kv] read: contract ${contractID} has no resolved type`);
        }
        const slot = this.kvSlots.get(registryKey(contractType, key));
        if (!slot) {
            throw new errors_js_1.ChelErrorKvSlotUnknown(`[chelonia/kv] read: no slot registered for ${contractType}::${key}`);
        }
    ...
    'chelonia/kv/sync': async function (contractID, key) {
        ...
        const contractMeta = rootState.contracts?.[contractID];
        if (!contractMeta || !this.subscriptionSet.has(contractID)) {
            throw new errors_js_1.ChelErrorKvSlotUnknown(`[chelonia/kv] sync: contract ${contractID} is not synced`);
        }
        const contractType = contractMeta.type;
        if (typeof contractType !== 'string') {
            throw new errors_js_1.ChelErrorKvSlotUnknown(`[chelonia/kv] sync: contract ${contractID} has no resolved type`);
        }
        if (key !== undefined) {
            const slot = this.kvSlots.get(registryKey(contractType, key));
            if (!slot) {
                throw new errors_js_1.ChelErrorKvSlotUnknown(`[chelonia/kv] sync: no slot registered for ${contractType}::${key}`);
            }
    ...
    'chelonia/kv/clear': async function (contractID, key) {
        const rootState = (0, sbp_1.default)(this.config.stateSelector);
        const contractMeta = rootState.contracts?.[contractID];
        if (!contractMeta || !this.subscriptionSet.has(contractID)) {
            throw new errors_js_1.ChelErrorKvSlotUnknown(`[chelonia/kv] clear: contract ${contractID} is not synced`);
        }
        const contractType = contractMeta.type;
        if (typeof contractType !== 'string') {
            throw new errors_js_1.ChelErrorKvSlotUnknown(`[chelonia/kv] clear: contract ${contractID} has no resolved type`);
        }
        const slot = this.kvSlots.get(registryKey(contractType, key));
        if (!slot) {
            throw new errors_js_1.ChelErrorKvSlotUnknown(`[chelonia/kv] clear: no slot registered for ${contractType}::${key}`);
        }
        const nonce = base64Nonce();
        recordEchoNonce(this, contractID, key, nonce);
