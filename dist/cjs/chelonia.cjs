...
// './keys.cjs' registers the key API selectors; it must be imported before
...
require("./keys.cjs");
const keys_js_1 = require("./keys.cjs");
...
exports.default = (0, sbp_1.default)('sbp/selectors/register', {
    ...
    'chelonia/configure': async function (config) {
        ...
        (0, turtledash_1.merge)(this.config, configForMerge);
        ...
        if (journalOverride === null) {
            ...
            const rejectNull = (name) => {
                ...
                if ((0, turtledash_1.has)(journalOverride, name) && journalOverride[name] === null) {
                    throw new TypeError(`[chelonia][journal] config.journal.${name} cannot be null; ` +
    ...
    'chelonia/contract/sync': async function (contractIDs, params) {
        ...
        return (0, sbp_1.default)('chelonia/private/in/sync', listOfIds, { ...params, force: true });
    },
    ...
    'chelonia/contract/retain': async function (contractIDs, params) {
        ...
        return await (0, sbp_1.default)('chelonia/private/in/sync', listOfIds);
    },
    ...
    'chelonia/contract/withRetained': async function (contractIDs, callback) {
        await (0, sbp_1.default)('chelonia/contract/retain', contractIDs, { ephemeral: true });
        try {
    ...
    'chelonia/out/nameToContractID': async function (name, { throwOnInvalidName } = {}) {
        ...
        if (response.status === 404 || response.status === 410)
            return null;
        if (!response.ok) {
            throw new errors_js_1.ChelErrorUnexpectedHttpResponseCode((0, utils_js_1.httpErrorMessage)(response), { cause: response.status });
        }
    ...
    'chelonia/out/registerContract': async function (params) {
        ...
        if (specParams != null) {
            ...
            signingKeyId = (0, keys_js_1.resolveGeneratedKeyReference)(keyMap, specParams.signingKeyId ?? null, specParams.signingKeyName ?? null, 'signingKey');
            actionSigningKeyId = (0, keys_js_1.resolveGeneratedKeyReference)(keyMap, specParams.actionSigningKeyId ?? null, specParams.actionSigningKeyName ?? null, 'actionSigningKey');
            actionEncryptionKeyId = specParams.actionEncryptionKeyId != null ||
                specParams.actionEncryptionKeyName != null
                ? (0, keys_js_1.resolveGeneratedKeyReference)(keyMap, specParams.actionEncryptionKeyId ?? null, specParams.actionEncryptionKeyName ?? null, 'actionEncryptionKey')
                : undefined;
        ...
        await (0, sbp_1.default)('chelonia/private/in/sync', contractID);
    ...
    'chelonia/out/keyShare': async function (params) {
        ...
        let msg = SPMessage_js_1.SPMessage.createV1_0({
            ...
            op: [
                ...
                signingKeyId
                    ? (0, signedData_js_1.signedOutgoingData)(contractID, signingKeyId, payload, this.transientSecretKeys)
    ...
    'chelonia/out/shareKeys': async function (params) {
        ...
        if (subjectContractID === contractID)
            return;
        return await (0, sbp_1.default)('chelonia/contract/withRetained', [contractID, subjectContractID], async () => {
            const rootState = (0, sbp_1.default)(this.config.stateSelector);
            ...
            const destEncryptionKeyId = (encryptionKeyId != null || encryptionKeyName != null
                ? (0, keys_js_1.resolveStateKeyReference)(destState, encryptionKeyId ?? null, encryptionKeyName ?? null, 'encryptionKey')
                : (0, keys_js_1.resolveStateKeyReference)(destState, null, 'cek', 'encryptionKey'));
            ...
            let resolvedSigningKeyId = signingKeyId != null || signingKeyName != null
                ? (0, keys_js_1.resolveStateKeyReference)(destState, signingKeyId ?? null, signingKeyName ?? null, 'signingKey')
                : undefined;
            if (resolvedSigningKeyId == null) {
                resolvedSigningKeyId =
                    (0, utils_js_1.findSuitableSecretKeyId)(destState, [SPMessage_js_1.SPMessage.OP_KEY_SHARE], ['sig']) ?? undefined;
            }
            ...
            if (keyNames != null && keyNames !== '*') {
                selected = keyNames.map((name) => {
                    const keyId = (0, utils_js_1.findKeyIdByName)(subjectState, name);
                    const k = keyId != null ? subjectState._vm.authorizedKeys[keyId] : undefined;
                    if (!k || !isSelectable(k)) {
                        throw new errors_js_1.ChelErrorKeyNameNotFound(`chelonia/out/shareKeys: no active recoverable key named '${name}' ` +
                ...
                selected = keyIds.map((id) => {
                    ...
                    if (!k || !isSelectable(k)) {
                        throw new errors_js_1.ChelErrorKeyNameNotFound(`chelonia/out/shareKeys: no active recoverable key with ID '${id}' ` +
                            `in subject contract ${subjectContractID}`);
                    }
                    return k;
                });
            ...
            const sharedKeys = selected.map((k) => {
                ...
                const secret = (0, crypto_1.serializeKey)(rawKey, true);
                ...
                return {
                    ...
                    meta: {
                        private: {
                            content: (0, encryptedData_js_1.encryptedOutgoingData)(contractID, destEncryptionKeyId, secret)
                        }
                    }
                };
            });
            ...
            return await (0, sbp_1.default)('chelonia/out/keyShare', {
                contractID,
                contractName,
                originatingContractID: subjectContractID,
                originatingContractName: subjectContractTypeName,
                data: (0, encryptedData_js_1.encryptedOutgoingData)(contractID, destEncryptionKeyId, payload),
    ...
    'chelonia/out/keyAdd': async function (params) {
        ...
        if (!Array.isArray(params.data)) {
            ...
            const keyMap = (await (0, sbp_1.default)('chelonia/key/generate', {
                contractID,
                keys: params.data
            }));
            data = Object.values(keyMap).map((k) => k.spkey);
        }
        else if (params.data.some((entry) => (0, keys_js_1.isKeySpec)(entry))) {
            const markedSpecs = params.data.filter((entry) => (0, keys_js_1.isKeySpec)(entry));
            const keyMap = (await (0, sbp_1.default)('chelonia/key/generate', {
                contractID,
                keys: markedSpecs
            }));
            data = params.data.map((entry) => (0, keys_js_1.isKeySpec)(entry) ? keyMap[entry.alias].spkey : entry);
        }
        else {
            data = params.data;
        }
    ...
    'chelonia/out/keyUpdate': async function (params) {
        ...
        let data;
        if (!Array.isArray(params.data)) {
            const { updates, newKeys } = (0, keys_js_1.expandKeyUpdateSpecs)({
                updates: params.data,
                contractID,
                contractState: state
            });
            (0, keys_js_1.storeGeneratedSecretKeys)(this, Object.values(newKeys));
            data = updates;
        }
        else if (params.data.some((entry) => (0, keys_js_1.isKeyUpdateSpec)(entry))) {
            const markedSpecs = params.data.filter((entry) => (0, keys_js_1.isKeyUpdateSpec)(entry));
            const { updates, newKeys } = (0, keys_js_1.expandKeyUpdateSpecs)({
                updates: markedSpecs,
                contractID,
                contractState: state
            ...
            let specIndex = 0;
            data = params.data.map((entry) => (0, keys_js_1.isKeyUpdateSpec)(entry)
                ? updates[specIndex++]
                : entry);
        }
        else {
    ...
    'chelonia/out/keyRequest': async function (params) {
        ...
        return await (0, sbp_1.default)('chelonia/contract/withRetained', contractID, async () => {
            ...
            if (keyRequestResponseId) {
                ...
                const keyAddSigningKeyId = (0, utils_js_1.findSuitableSecretKeyId)(originatingState, [SPMessage_js_1.SPMessage.OP_KEY_ADD], ['sig']);
            ...
            let msg = SPMessage_js_1.SPMessage.createV1_0({
                ...
                op: [
                    ...
                    (0, signedData_js_1.signedOutgoingData)(contractID, signingKeyId, {
    ...
    'chelonia/out/atomic': async function (params) {
        ...
        if (params.originatingContractID != null ||
            params.originatingContractName != null) {
            throw new TypeError('chelonia/out/atomic no longer accepts originatingContractID / ' +
...
const isSpecRegistration = (params) => {
    ...
    if (Array.isArray(keys))
        return keys.some((k) => (0, keys_js_1.isKeySpec)(k));
...
const expandRegistrationKeys = async function (params) {
    ...
    if (params.autoSak) {
        ...
        if (typeof params.autoSak.encryptWith !== 'string' || !params.autoSak.encryptWith) {
            throw new errors_js_1.ChelErrorKeySpecInvalid('autoSak requires an explicit encryptWith wrapper name');
        }
        const entries = (0, keys_js_1.normalizeKeySpecs)(keys);
        ...
        if (!hasSak) {
            ...
            keys = Array.isArray(keys)
                ? [...keys, (0, keys_js_1.keySpec)('#sak', sakSpec['#sak'])]
                : { ...keys, ...sakSpec };
        }
    }
...
const resolveSigningKeyId = function (contractID, params) {
    return resolveRef.call(this, contractID, { id: params.signingKeyId, name: params.signingKeyName }, 'signingKey');
};
...
async function outEncryptedOrUnencryptedAction(opType, params) {
    ...
    const signedMessage = innerSigningKeyId
        ? state._vm.authorizedKeys[innerSigningKeyId] &&
            state._vm.authorizedKeys[innerSigningKeyId]?._notAfterHeight == null
            ? (0, signedData_js_1.signedOutgoingData)(contractID, innerSigningKeyId, unencMessage, this.transientSecretKeys)
            : (0, signedData_js_1.signedOutgoingDataWithRawKey)(this.transientSecretKeys[innerSigningKeyId], unencMessage)
        : unencMessage;
    if (opType === SPMessage_js_1.SPMessage.OP_ACTION_ENCRYPTED && !encryptionKeyId && !params.encryptionKey) {
        throw new Error('OP_ACTION_ENCRYPTED requires an encryption key ID be given');
    }
    if (params.encryptionKey) {
        if (encryptionKeyId !== (0, crypto_1.keyId)(params.encryptionKey)) {
