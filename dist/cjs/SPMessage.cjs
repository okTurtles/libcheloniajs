        ...
        return (0, encryptedData_js_1.maybeEncryptedIncomingData)(contractID, state, message, height, additionalKeys, headJSON, (message) => {
            message.keys?.forEach((key) => {
                ...
                const decryptionFn = message.foreignContractID
                    ? encryptedData_js_1.encryptedIncomingForeignData
                    : encryptedData_js_1.encryptedIncomingData;
...
class SPMessage {
    ...
    static deserialize(value, additionalKeys, state, unwrapMaybeEncryptedDataFn = encryptedData_js_1.unwrapMaybeEncryptedData) {
        ...
        const contractID = head.op === SPMessage.OP_CONTRACT
            ? (0, functions_js_1.createCID)(value, functions_js_1.multicodes.SHELTER_CONTRACT_DATA)
            : head.contractID;
