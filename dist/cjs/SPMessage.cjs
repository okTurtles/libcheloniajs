...
const decryptedAndVerifiedDeserializedMessage = (head, headJSON, contractID, parsedMessage, additionalKeys, state) => {
    ...
    const message = op === SPMessage.OP_ACTION_ENCRYPTED
        ? (0, encryptedData_js_1.encryptedIncomingData)(contractID, state, parsedMessage, height, additionalKeys, headJSON, undefined)
        : parsedMessage;
    ...
    if ([SPMessage.OP_KEY_ADD, SPMessage.OP_KEY_UPDATE].includes(op)) {
        return message.map((key) => {
            return (0, encryptedData_js_1.maybeEncryptedIncomingData)(contractID, state, key, height, additionalKeys, headJSON, (key) => {
                if (key.meta?.private?.content) {
                    key.meta.private.content = (0, encryptedData_js_1.encryptedIncomingData)(contractID, state, key.meta.private.content, height, additionalKeys, headJSON, (value) => {
                        // Validator function to verify the key matches its expected ID
                        const computedKeyId = (0, crypto_1.keyId)(value);
                ...
                if (key.meta?.keyRequest?.reference) {
                    try {
                        key.meta.keyRequest.reference = (0, encryptedData_js_1.maybeEncryptedIncomingData)(contractID, state, key.meta.keyRequest.reference, height, additionalKeys, headJSON)?.valueOf();
                    }
                    catch {
                ...
                if (key.meta?.keyRequest?.contractID) {
                    try {
                        key.meta.keyRequest.contractID = (0, encryptedData_js_1.maybeEncryptedIncomingData)(contractID, state, key.meta.keyRequest.contractID, height, additionalKeys, headJSON)?.valueOf();
                    }
                    catch {
                        // If we couldn't decrypt it, this value is of no use to us (we
                        // can't keep track of key requests and key shares), so we delete it
    ...
    if (op === SPMessage.OP_CONTRACT) {
        message.keys = message.keys?.map((key) => {
            return (0, encryptedData_js_1.maybeEncryptedIncomingData)(contractID, state, key, height, additionalKeys, headJSON, (key) => {
                ...
                const decryptionFn = encryptedData_js_1.encryptedIncomingData;
    ...
    if (op === SPMessage.OP_KEY_SHARE) {
        return (0, encryptedData_js_1.maybeEncryptedIncomingData)(contractID, state, message, height, additionalKeys, headJSON, (message) => {
            message.keys?.forEach((key) => {
                if (!key.meta?.private?.content)
                    return;
                const decryptionFn = message.foreignContractID ? encryptedData_js_1.encryptedIncomingForeignData : encryptedData_js_1.encryptedIncomingData;
    ...
    if (op === SPMessage.OP_ACTION_UNENCRYPTED && (0, signedData_js_1.isRawSignedData)(message)) {
        return (0, signedData_js_1.signedIncomingData)(contractID, state, message, height, headJSON);
    }
    ...
    if (op === SPMessage.OP_KEY_DEL) {
        return message.map((key) => {
            return (0, encryptedData_js_1.maybeEncryptedIncomingData)(contractID, state, key, height, additionalKeys, headJSON, undefined);
        });
    }
...
class SPMessage {
    ...
    static deserialize(value, additionalKeys, state, unwrapMaybeEncryptedDataFn = encryptedData_js_1.unwrapMaybeEncryptedData) {
        if (!value)
        ...
        const contractID = head.op === SPMessage.OP_CONTRACT ? (0, functions_js_1.createCID)(value, functions_js_1.multicodes.SHELTER_CONTRACT_DATA) : head.contractID;
        ...
        if (!state?._vm?.authorizedKeys && head.op === SPMessage.OP_CONTRACT) {
            const value = (0, signedData_js_1.rawSignedIncomingData)(parsedValue);
        ...
        const signedMessageData = (0, signedData_js_1.signedIncomingData)(contractID, state, parsedValue, head.height, headJSON, (message) => decryptedAndVerifiedDeserializedMessage(head, headJSON, contractID, message, additionalKeys, state));
        return new this({
            direction: 'incoming',
            mapping: { key: (0, functions_js_1.createCID)(value, functions_js_1.multicodes.SHELTER_CONTRACT_DATA), value },
            head,
            signedMessageData
        });
    }
    static deserializeHEAD(value) {
        ...
        const result = {
            ...
            get hash() {
                if (!hash) {
                    hash = (0, functions_js_1.createCID)(value, functions_js_1.multicodes.SHELTER_CONTRACT_DATA);
                }
                return hash;
            },
            get contractID() {
    ...
    decryptedValue() {
        ...
        try {
            ...
            const data = (0, encryptedData_js_1.unwrapMaybeEncryptedData)(value);
            ...
            if (data?.data) {
                ...
                if ((0, signedData_js_1.isSignedData)(data.data)) {
    ...
    static [serdes_1.serdesSerializeSymbol](m) {
        return [m.serialize(), m.direction(), m.decryptedValue(), m.innerSigningKeyId()];
    }
    static [serdes_1.serdesDeserializeSymbol]([serialized, direction, decryptedValue, innerSigningKeyId]) {
...
function messageToParams(head, message) {
    ...
    let mapping;
    return {
        direction: (0, turtledash_1.has)(message, 'recreate') ? 'outgoing' : 'incoming',
        ...
        get mapping() {
            if (!mapping) {
                ...
                mapping = {
                    key: (0, functions_js_1.createCID)(value, functions_js_1.multicodes.SHELTER_CONTRACT_DATA),
                    value
                };
            }
            return mapping;
